...
typedef void AtomicThreeOpFn(TCGContext *, TCGv_i64, TCGv_cap_checked_ptr, TCGv_i64, TCGArg, MemOp);
...
void a64_translate_init(struct uc_struct *uc)
{
...
#ifdef TARGET_CHERI
    tcg_ctx->cpu_pc_arm64 = tcg_global_mem_new(tcg_ctx, tcg_ctx->cpu_env,
                                offsetof(CPUARMState, pc.cap._cr_cursor), "pc");
    // Morello sometimes interposes with base
    tcg_ctx->ddc_interposition = tcg_global_mem_new(tcg_ctx,
        tcg_ctx->cpu_env, offsetof(CPUARMState, DDC_current.cap.cr_base), "ddc_base");
#else
    ...
    for (i = 0; i < 32; i++) {
#ifdef TARGET_CHERI
        tcg_ctx->_cpu_cursors_do_not_access_directly[i] = tcg_global_mem_new(
...
static void gen_a64_set_pc_im_safe(DisasContext *s, uint64_t val)
{
    ...
    if (!in_pcc_bounds(&s->base, val)) {
        TCGv_i64 tcgval = tcg_const_i64(tcg_ctx, val);
        gen_helper_set_pcc(tcg_ctx, tcg_ctx->cpu_env, tcgval);
        tcg_temp_free_i64(tcg_ctx, tcgval);
        return;
    }
#endif
    gen_a64_set_pc_im(tcg_ctx, val);
}
...
static void gen_a64_set_link_register(DisasContext *s)
{
    ...
    tcg_gen_movi_i64(tcg_ctx, cpu_reg(s, 30), addr);
    if (c64)
...
static void gen_a64_set_pc(DisasContext *s, TCGv_i64 src)
{
    ...
    if (have_cheri_tb_flags(s, TB_FLAG_CHERI_PCC_FULL_AS)) {
        ...
        TCGv_i64 tbi_dst = tcg_temp_local_new_i64(tcg_ctx);
        TCGv_i64 skip_rep_check = tcg_temp_new_i64(tcg_ctx);
        ...
        if (!have_cheri_tb_flags(s, TB_FLAG_CHERI_PCC_BASE_ZERO)) {
            tcg_gen_setcondi_i64(tcg_ctx, TCG_COND_GEU, skip_rep_check, tbi_dst,
                                 s->base.pcc_base);
            need_and = true;
        }

        if (!have_cheri_tb_flags(s, TB_FLAG_CHERI_PCC_TOP_MAX)) {
            TCGv_i64 tmp = need_and ? tcg_temp_new_i64(tcg_ctx) : NULL;
            tcg_gen_setcondi_i64(tcg_ctx, TCG_COND_LEU, need_and ? tmp : skip_rep_check,
                                 tbi_dst, s->base.pcc_top);
            if (need_and) {
                tcg_gen_and_i64(tcg_ctx, skip_rep_check, skip_rep_check, tmp);
                tcg_temp_free_i64(tcg_ctx, tmp);
            }
        }

        TCGLabel *skip_representability_check_lbl = gen_new_label(tcg_ctx);
        tcg_gen_brcondi_tl(tcg_ctx, TCG_COND_NE, skip_rep_check, 0,
                           skip_representability_check_lbl);
        gen_helper_set_pcc(tcg_ctx, tcg_ctx->cpu_env, tbi_dst);
        gen_set_label(tcg_ctx, skip_representability_check_lbl);
        tcg_gen_mov_i64(tcg_ctx, tcg_ctx->cpu_pc_arm64, tbi_dst);
        tcg_temp_free_i64(tcg_ctx, tbi_dst);
        tcg_temp_free_i64(tcg_ctx, skip_rep_check);
    }
#else
    gen_top_byte_ignore(s, s->uc->tcg_ctx->cpu_pc_arm64, src, s->tbii);
#endif
                                  ...
                                  TCGv_i64 new_value, bool capability_base)
{
    ...
    if (capability_base) {
        ...
        tcg_gen_mov_i64(tcg_ctx, cpu_reg_sp(s, regnum), new_value);
                                              ...
                                              bool ddc_base)
{
    ...
    // refactoring callers
    TCGv_i64 save = tcg_temp_local_new_i64(tcg_ctx);
    tcg_gen_mov_i64(tcg_ctx, save, addr);
    TCGv_cap_checked_ptr result = arm_bounds_checked(
        s, clean, size, base_reg, is_load, is_store, alternate_base, ddc_base);
    tcg_gen_mov_i64(tcg_ctx, addr, save);
    tcg_temp_free_i64(tcg_ctx, save);
    return result;
}
...
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    TCGv_i64 tfar = tcg_const_i64(tcg_ctx, far);
    tcg_gen_st_i64(tcg_ctx, tfar, tcg_ctx->cpu_env, offsetof(CPUArchState, exception.vaddress));
    tcg_temp_free_i64(tcg_ctx, tfar);
}
...
{
#ifdef TARGET_CHERI
    return s->uc->tcg_ctx->_cpu_cursors_do_not_access_directly[reg];
#else
...
TCGv_i64 read_cpu_reg_sp(DisasContext *s, int reg, int sf)
{
    ...
    TCGv_i64 *ptr;
#ifdef TARGET_CHERI
    ptr = &tcg_ctx->_cpu_cursors_do_not_access_directly[reg];
#else
    ptr = &tcg_ctx->cpu_X[reg];
#endif
    if (sf) {
        tcg_gen_mov_i64(tcg_ctx, v, *ptr);
    } else {
        tcg_gen_ext32u_i64(tcg_ctx, v, *ptr);
    }
    return v;
}
...
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    tcg_gen_qemu_st_i64_with_checked_addr(tcg_ctx,
                             ...
                             bool iss_ar)
{
    ...
    tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, dest, tcg_addr, memidx, memop);
                     ...
                     int size)
{
    ...
    if (size < 4) {
        tcg_gen_qemu_st_i64_with_checked_addr(tcg_ctx, tmp, tcg_addr, get_mem_index(s),
                                              memop + size | align);
    } else {
        bool be = s->be_data == MO_BE;
        TCGv_cap_checked_ptr tcg_hiaddr = tcg_temp_new_cap_checked(tcg_ctx);

        tcg_gen_addi_i64(tcg_ctx, (TCGv_i64)tcg_hiaddr, (TCGv_i64)tcg_addr, 8);
        // If the first load is correctly aligned, so will the second
        tcg_gen_qemu_st_i64_with_checked_addr(tcg_ctx, tmp, be ? tcg_hiaddr : tcg_addr,
                                              get_mem_index(s),
                                              memop | MO_Q | align);
        tcg_gen_ld_i64(tcg_ctx, tmp, tcg_ctx->cpu_env, fp_reg_hi_offset(s, srcidx));
        tcg_gen_qemu_st_i64_with_checked_addr(tcg_ctx, tmp, be ? tcg_addr : tcg_hiaddr,
                                              get_mem_index(s), memop | MO_Q);
        tcg_temp_free_cap_checked(tcg_ctx, tcg_hiaddr);
    }
 ...
 */
static void do_fp_ld(DisasContext *s, int destidx, TCGv_cap_checked_ptr tcg_addr, int size)
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    ...
    if (size < 4) {
        tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, tmplo, tcg_addr, get_mem_index(s),
                                              memop + size | align);
    } else {
        bool be = s->be_data == MO_BE;
        ...
        tcg_hiaddr = tcg_temp_new_cap_checked(tcg_ctx);

        tcg_gen_addi_i64(tcg_ctx, (TCGv_i64)tcg_hiaddr, (TCGv_i64)tcg_addr, 8);
        tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, tmplo, be ? tcg_hiaddr : tcg_addr,
                                              get_mem_index(s),
                                              memop | MO_Q | align);
        tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, tmphi, be ? tcg_addr : tcg_hiaddr,
                                              get_mem_index(s), memop | MO_Q);
        tcg_temp_free_cap_checked(tcg_ctx, tcg_hiaddr);
    }
                      ...
                      TCGv_cap_checked_ptr tcg_addr, int size, MemOp endian)
{
    ...
    tcg_gen_qemu_st_i64_with_checked_addr(tcg_ctx, tcg_tmp, tcg_addr, get_mem_index(s),
                      ...
                      TCGv_cap_checked_ptr tcg_addr, int size, MemOp endian)
{
    ...
    tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, tcg_tmp, tcg_addr, get_mem_index(s),
                                          endian | size | memop_align_sctlr(s));
...
static inline void gen_check_sp_alignment(DisasContext *s)
{
    ...
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    if (get_sctlr_sa(s)) {
        TCGLabel *label_aligned = gen_new_label(tcg_ctx);
        TCGv_i64 align = tcg_const_i64(tcg_ctx, 16 - 1);
        tcg_gen_and_i64(tcg_ctx, align, align, cpu_reg_sp(s, 31));
        tcg_gen_brcondi_i64(tcg_ctx, TCG_COND_EQ, align, 0, label_aligned);
        gen_a64_set_pc_im(tcg_ctx, s->pc_curr);
        gen_helper_sp_alignment_exception(tcg_ctx, tcg_ctx->cpu_env);
        gen_set_label(tcg_ctx, label_aligned);
        tcg_temp_free_i64(tcg_ctx, align);
    }
}
...
#ifdef TARGET_CHERI
static inline bool capabilities_enabled_exception(DisasContext *s)
{
    uc_engine *uc = s->uc;
    TCGContext *tcg_ctx = uc->tcg_ctx;
    // Flag bits only encode an exception is needed, not which one. The helper
    // will do that.
    if (!GET_FLAG(s, CAP_ENABLED)) {
        s->base.is_jmp = DISAS_NORETURN;
        gen_a64_set_pc_im(tcg_ctx, s->pc_curr);
        gen_helper_check_capabilities_enabled_exception(tcg_ctx, tcg_ctx->cpu_env);
        return true;
    }
    return false;
}
                                                  ...
                                                  bool is_zva)
{
    ...
    tcg_gen_andi_i64(tcg_ctx, aligned, addr, ~(ZVA_SIZE - 1));
    ...
    if (!is_zva) {
        ...
        // after
        tmp32 = tcg_const_i32(tcg_ctx, 1);
        tcg_gen_st8_i32(tcg_ctx, tmp32, tcg_ctx->cpu_env, offsetof(CPUARMState, exception.cm));
    }
    ...
    if (!is_zva) {
        tcg_gen_movi_i32(tcg_ctx, tmp32, 0);
        tcg_gen_st8_i32(tcg_ctx, tmp32, tcg_ctx->cpu_env, offsetof(CPUARMState, exception.cm));
                       ...
                       unsigned int crn, unsigned int crm, unsigned int rt)
{
    ...
    if (!(ri->access & PL_NO_SYSREG) && !cheri_is_system_ctx(s)) {
        s->base.is_jmp = DISAS_NORETURN;
        gen_a64_set_pc_im(tcg_ctx, s->pc_curr);
        TCGv_i32 syndrome = tcg_const_i32(tcg_ctx, syn_aa64_sysregtrap_impl(
            op0, op1, op2, crn, crm, rt, isread, is_morello));
        gen_helper_sys_not_accessible_exception(tcg_ctx, tcg_ctx->cpu_env, syndrome);
        tcg_temp_free_i32(tcg_ctx, syndrome);
        return;
    }
    ...
    case ARM_CP_DC_ZVA:
        ...
        clean_addr = bounds_check_cache_op(s, cpu_reg(s, rt), rt, false, true, true);
    ...
    if (is_morello) {
        ...
        if (isread) {
            if (ri->type & ARM_CP_CONST) {
                ...
                TCGv_i32 regno = tcg_const_i32(tcg_ctx, rt);
                gen_helper_get_cp_cap(tcg_ctx, tcg_rt, tcg_ctx->cpu_env, tmpptr, regno);
                tcg_temp_free_i32(tcg_ctx, regno);
                tcg_temp_free_ptr(tcg_ctx, tmpptr);
            } else {
            ...
            if (ri->type & ARM_CP_CONST) {
                ...
                TCGv_ptr tmpptr = tcg_const_ptr(tcg_ctx, ri);
                TCGv_i32 regno = tcg_const_i32(tcg_ctx, rt);
                gen_helper_set_cp_cap(tcg_ctx, tcg_ctx->cpu_env, tmpptr, tcg_rt, regno);
                tcg_temp_free_i32(tcg_ctx, regno);
                tcg_temp_free_ptr(tcg_ctx, tmpptr);
            } else {
...
static void disas_uncond_b_reg(DisasContext *s, uint32_t insn)
{
    ...
    case 2: /* RET */
        ...
        if (cctlr_set(s, CCTLR_PCCBO)) {
            ...
            tcg_gen_movi_i64(tcg_ctx, dst_tmp, s->base.pcc_base);
            tcg_gen_add_i64(tcg_ctx, dst_tmp, dst_tmp, dst);
            dst = dst_tmp;
        }
#endif
                               ...
                               bool is_pair)
{
    ...
    if (is_pair) {
        ...
        if (size == 2) {
            ...
            tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, tcg_ctx->cpu_exclusive_val, addr, idx,
                                                  memop);
            if (s->be_data == MO_LE) {
            ...
            TCGv_cap_checked_ptr addr2 = tcg_temp_new_cap_checked(tcg_ctx);
            tcg_gen_addi_i64(tcg_ctx, (TCGv_i64)addr2, (TCGv_i64)addr, 8);
            tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, tcg_ctx->cpu_exclusive_high, addr2,
                                                  idx, memop);
            tcg_temp_free_cap_checked(tcg_ctx, addr2);
    ...
    tcg_gen_mov_i64(tcg_ctx, tcg_ctx->cpu_exclusive_addr, (TCGv_i64)addr);
}
                                ...
                                int is_pair)
{
    ...
    TCGv_i64 tmp = tcg_temp_local_new_i64(tcg_ctx);
    tcg_gen_mov_i64(tcg_ctx, tmp, (TCGv_i64)addr);

    tcg_gen_brcond_i64(tcg_ctx, TCG_COND_NE, (TCGv_i64)addr, tcg_ctx->cpu_exclusive_addr,
                       fail_label);
    ...
    if (is_pair) {
        if (size == 2) {
            ...
            tcg_gen_atomic_cmpxchg_i64_with_checked_addr(
                tcg_ctx, tmp, (TCGv_cap_checked_ptr)tcg_ctx->cpu_exclusive_addr,
        ...
        tcg_gen_atomic_cmpxchg_i64_with_checked_addr(
            tcg_ctx, tmp, (TCGv_cap_checked_ptr)tcg_ctx->cpu_exclusive_addr, tcg_ctx->cpu_exclusive_val,
    ...
    TCGLabel *align_fail_label = gen_new_label(tcg_ctx);
    tcg_gen_andi_i64(tcg_ctx, (TCGv_i64)addr, tmp, (1 << size) - 1);
    tcg_gen_brcondi_i64(tcg_ctx, TCG_COND_EQ, (TCGv_i64)addr, 0, align_fail_label);
    gen_helper_alignment_fault_exception(tcg_ctx, tcg_ctx->cpu_env, tmp);
    gen_set_label(tcg_ctx, align_fail_label);
#endif
    tcg_temp_free_i64(tcg_ctx, tmp);
                                 ...
                                 int rn, int size)
{
    ...
    clean_addr = clean_data_tbi_and_cheri(s, cpu_reg_sp(s, rn), true, true,
                                          1 << size, rn, false, true);

    TCGv_i64 tcg_rs = cpu_reg(s, rs);
    TCGv_i64 tcg_rt = cpu_reg(s, rt);

    tcg_gen_atomic_cmpxchg_i64_with_checked_addr(tcg_ctx, tcg_rs, clean_addr, tcg_rs,
                                                 tcg_rt, memidx,
                                      ...
                                      int rn, int size)
{
    ...
    if (size == 2) {
        ...
        tcg_gen_atomic_cmpxchg_i64_with_checked_addr(
            tcg_ctx, cmp, clean_addr, cmp, val, memidx, MO_64 | MO_ALIGN | s->be_data);
        ...
        tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, d1, clean_addr, memidx,
                                              MO_64 | MO_ALIGN_16 | s->be_data);
        tcg_gen_addi_i64(tcg_ctx, a2, (TCGv_i64)clean_addr, 8);
        tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, d2, (TCGv_cap_checked_ptr)a2,
        ...
        */
        TCGv_i64 checked_addr = tcg_temp_new_i64(tcg_ctx);
        TCGv_i32 store_happens = tcg_temp_new_i32(tcg_ctx);
        tcg_gen_extrl_i64_i32(tcg_ctx, store_happens, c2);
        tcg_gen_addi_i64(tcg_ctx, checked_addr, (TCGv_i64)clean_addr, 4);
        handle_conditional_invalidate(tcg_ctx, (TCGv_cap_checked_ptr)checked_addr, MO_64,
                                      memidx, store_happens);
        tcg_temp_free_i64(tcg_ctx, checked_addr);
        tcg_temp_free_i32(tcg_ctx, store_happens);
#endif
        ...
        tcg_gen_qemu_st_i64_with_checked_addr_cond_invalidate(
            tcg_ctx, c1, clean_addr, memidx, MO_64 | s->be_data, false);
        tcg_gen_qemu_st_i64_with_checked_addr_cond_invalidate(
            tcg_ctx, c2, (TCGv_cap_checked_ptr)a2, memidx, MO_64 | s->be_data, false);
...
static void disas_ldst_excl(DisasContext *s, uint32_t insn)
{
    ...
    case 0x1: /* STLXR */
        ...
        clean_addr = clean_data_tbi_and_cheri(s, cpu_reg_sp(s, rn), false, true,
                                              1 << size, rn, false, true);
    ...
    case 0x5: /* LDAXR */
        ...
        clean_addr = clean_data_tbi_and_cheri(s, cpu_reg_sp(s, rn), true, false,
                                              1 << size, rn, false, true);
        s->is_ldex = true;
    ...
    case 0x2: case 0x3: /* CASP / STXP */
        if (size & 2) { /* STXP / STLXP */
            ...
            clean_addr =
                clean_data_tbi_and_cheri(s, cpu_reg_sp(s, rn), false, true,
                                         1 << (size + 1), rn, false, true);
    ...
    case 0x6: case 0x7: /* CASPA / LDXP */
        if (size & 2) { /* LDXP / LDAXP */
            ...
            clean_addr =
                clean_data_tbi_and_cheri(s, cpu_reg_sp(s, rn), true, false,
                                         1 << (size + 1), rn, false, true);
            s->is_ldex = true;
...
static void disas_ld_lit(DisasContext *s, uint32_t insn)
{
    ...
    dirty_addr = tcg_const_i64(tcg_ctx, s->pc_curr + imm);
...
static void disas_ldst_pair(DisasContext *s, uint32_t insn)
{
    ...
    clean_addr = clean_data_tbi_and_cheri(s, dirty_addr, is_load, !is_load,
    ...
    if (is_vector) {
        ...
        tcg_gen_addi_i64(tcg_ctx, (TCGv_i64)clean_addr, (TCGv_i64)clean_addr, 1ULL << size);
        if (is_load) {
        ...
        if (is_load) {
            ...
            tcg_gen_addi_i64(tcg_ctx, (TCGv_i64)clean_addr, (TCGv_i64)clean_addr,
                             1 << size);
                                ...
                                bool is_vector)
{
    ...
    clean_addr = clean_data_tbi_and_cheri(
        s, dirty_addr, !is_store, is_store, size, rn, false, true);
                              ...
                              int size, int rt, bool is_vector)
{
    ...
    clean_addr =
        clean_data_tbi_and_cheri(s, cpu_reg_sp(s, rn), true, o3_opc != 014,
                                 size, rn, false, true);
                           ...
                           int size, int rt, bool is_vector)
{
    ...
    clean_addr =
        clean_data_tbi_and_cheri(s, dirty_addr, true, false,
                                 size, rn, false, true);
...
static void disas_ldst_multiple_struct(DisasContext *s, uint32_t insn)
{
     ...
     */
    clean_addr = clean_data_tbi_and_cheri(s, tcg_rn, !is_store, is_store,
                                          total, rn, false, true);
    ...
    for (r = 0; r < rpt; r++) {
        ...
        for (e = 0; e < elements; e++) {
            ...
            for (xs = 0; xs < selem; xs++) {
                ...
                tcg_gen_add_i64(tcg_ctx, (TCGv_i64)clean_addr, (TCGv_i64)clean_addr, tcg_ebytes);
            }
        }
    }
    ...
    if (is_postidx) {
        TCGv_i64 tmp = tcg_temp_new_i64(tcg_ctx);
        if (rm == 31) {
            tcg_gen_addi_i64(tcg_ctx, tmp, tcg_rn, total);
        } else {
            tcg_gen_add_i64(tcg_ctx, tmp, tcg_rn, cpu_reg(s, rm));
        }
...
static void disas_ldst_single_struct(DisasContext *s, uint32_t insn)
{
    ...
    clean_addr = clean_data_tbi_and_cheri(s, tcg_rn, is_load, !is_load,
                                          total, rn, false, true);
    ...
    for (xs = 0; xs < selem; xs++) {
        if (replicate) {
            ...
            tcg_gen_qemu_ld_i64_with_checked_addr(tcg_ctx, 
...
static void aarch64_tr_tb_start(DisasContextBase *db, CPUState *cpu)
{
    ...
    if (db->pc_next & 0b11) {
        gen_set_exception_far(s, db->pc_next);
...
void cheri_tcg_save_pc(DisasContextBase *dcbase)
{
    DisasContext *dc = container_of(dcbase, DisasContext, base);
    TCGContext *tcg_ctx = dc->uc->tcg_ctx;
    gen_a64_set_pc_im(tcg_ctx, dcbase->pc_next);
}
